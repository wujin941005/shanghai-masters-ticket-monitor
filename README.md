...
- 场次目录名称由售票平台读取，但持续监控范围固定为进程启动时的
  `MATCH`；不会根据其他场次的售罄状态自动扩大。修改 `MATCH` 后需要重启进程。
- 使用 `MATCH` 选择场次，单次最多监控 20 个跨平台场次入口。默认的半决赛、决赛筛选约为 14 个入口。
...
本项目 WxPusher 使用个人模式，只接受一个 `WXPUSHER_UID`，不支持批量 UID、Topic、订单回调或用户数据库。先在 WxPusher 创建标准应用并关注该应用，再从 WxPusher 获取自己的 UID。需要多人运营或付费订单管理时，请自行设计权限、订阅期限与隐私保护，不要把个人配置直接公开。
...
本项目只内置 Bark、Server酱、单 UID WxPusher 和直连网络访问。
...
| `tests/` | 库存逻辑、通知格式以及项目边界的自动测试 |

项目只依赖 `monitor.py` 与 `notifications.py`。`.env`、运行日志、状态文件及 SQLite 文件均由 `.gitignore` 排除；发布前的边界测试也会检查这些规则以及示例配置中的密钥泄漏。
